...
<?php if (strtolower($content) != "collections" && strtolower($content) != "all" && strtolower($content) != "new"): ?>
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-x-2 sm:gap-x-5 p-4">
        <?php foreach ($results as $item): ?>
            <div class="flex flex-col cursor-pointer items"
                onclick="window.location.href='detail.php?id=<?= $item['id'] ?><?= isset($item['product_color']) ? '&color=' . $item['product_color'] : '&color=white' ?>'"
                data-name="<?= $item['product_name'] ?>" data-price="<?= $item['product_price'] ?>">
                <div class="h-[80%] flex justify-center items-end">
                    <img class="w-full h-full object-cover justify-center items-end bg-[#F3F3F3]"
...
<?php elseif (strtolower($content) == "collections"):
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-x-2 sm:gap-x-5 p-4">
        <?php foreach ($product as $item): ?>
            <div class="flex flex-col cursor-pointer items"
                onclick="window.location.href='detail.php?id=<?= $item['id'] ?>&color=<?= $item['product_color'] ?>'"
                data-name="<?= $item['product_name'] ?>" data-price="<?= $item['product_price'] ?>">
                <div class="h-[80%] flex justify-center items-end">
                    <img class="w-full h-full object-cover justify-center items-end bg-[#F3F3F3]"
                        src="../<?= $item['product_img'] ?>" alt="">
...
<?php elseif (strtolower($content) == "all"): ?>
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-x-2 sm:gap-x-5 p-4">
        <?php foreach ($all as $item): ?>
            <div class="flex flex-col cursor-pointer items"
                onclick="window.location.href='detail.php?id=<?= $item['id'] ?>&color=white'"
                data-name="<?= $item['product_name'] ?>" data-price="<?= $item['product_price'] ?>">
                <div class="h-[80%] flex justify-center items-end">
                    <img class="w-full h-full object-cover justify-center items-end bg-[#F3F3F3]"
                        src="../<?= $item['product_img'] ?>" alt="">
