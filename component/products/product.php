<?php foreach ($baseProduct as $base): ?>
    ...
    <div class="group cursor-pointer collections-child">
        <div class="relative bg-[#F3F3F3] aspect-[3/4] mb-4 overflow-hidden">
            <span
                class="absolute top-2 left-2 bg-white text-[9px] uppercase tracking-widest px-2 py-0.5 z-10">Limited</span>
            <img src="../<?= $base['product_img'] ?>"
                class="w-full h-full object-cover group-hover:scale-105 transition duration-500" alt="Product">
        </div>
...
<?php foreach ($product as $key => $item): ?>
    <?php if ($item['product_name'] !== "Summer Shirt" || $item['product_color'] === "beige") continue; ?>

    <div class="group cursor-pointer collections-child" data-id="<?= $item['id'] ?>" data-img="../<?= $item['product_img'] ?>"
        data-name="<?= $item['product_color'] ?> <?= $item['product_name'] ?>" data-price="<?= $item['product_price'] ?>"
        data-category="<?= $item['product_category'] ?>" data-color="<?= $item['product_color'] ?>"
        ...
        <h3 class="text-xs uppercase tracking-wider text-gray-900"><?= $item['product_color'] ?>     <?= $item['product_name'] ?>
        </h3>
